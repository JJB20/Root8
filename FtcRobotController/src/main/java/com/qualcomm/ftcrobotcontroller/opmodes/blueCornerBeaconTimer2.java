...
public class blueCornerBeaconTimer2 extends PushBotTelemetrySensors {
	...
	public void loop() {
		...
		switch (count) {
			case -3:
				...
				if (motorRight.getCurrentPosition() > currentPos + 6000) {
			...
			case 3:
				if (motorRight.getCurrentPosition() < currentPos + 4000) {
			...
			case 7:
				if (sensorTouch.isPressed()) {
					...
					motorRight.setPower(.0);
					currentPos = motorRight.getCurrentPosition();
					count = 16;//go to beacon business
				}
				else {
					if (sensorColor.blue() > 20) {
						motorLeft.setPower(vl * 0.8);
			...
			case 15:
				if (sensorTouch.isPressed()) {
					...
					if (sensorColor.blue() > 20) {//white
						motorLeft.setPower(vl * 0.75);
						motorRight.setPower(vr * 0.75);
					} else {//gray
						motorLeft.setPower(0);
						motorRight.setPower(vr * .75);
					}
				}
				break;
