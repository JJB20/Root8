...
public class blueMountainPark extends PushBotTelemetrySensors {
	...
	public void loop() {
		...
		telemetry.addData("MotorEncoder", motorRight.getCurrentPosition());
			...
			switch (count) {
				...
				//forward until robot passes corner of mountain
				case 0:
				...
				//turns to angle parallel to mountain
				case 1:
					motorLeft.setPower(vl);
					motorRight.setPower(-vr);
				...
				case 2:
					motorLeft.setPower(vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 1500) {
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;
				...
				case 3:
					motorLeft.setPower(vl);
					motorRight.setPower(-vr);
					if (motorRight.getCurrentPosition() < currentPos - 2200) {//90 degree turn
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;
