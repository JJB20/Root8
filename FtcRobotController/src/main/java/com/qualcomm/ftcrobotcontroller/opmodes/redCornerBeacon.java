...
public class redCornerBeacon extends PushBotTelemetrySensors {
	...
	public void init() {
		...
		currentPosSwipe = motorSwipe.getCurrentPosition();
	}
			...
			telemetry.addData("MotorEncoder difference", motorRight.getCurrentPosition() - currentPos);
				...
				switch (count) {
					...
					case 1:
						motorLeft.setPower(-vl);
						motorRight.setPower(vr);
						if (motorRight.getCurrentPosition() > currentPos + 1350) {
							currentPos = motorRight.getCurrentPosition();
							count++;
						}
						break;
					...
					case 6:
						...
						if (motorSwipe.getCurrentPosition() < currentPosSwipe - 100) {
