...
public class redCornerPark extends PushBotTelemetrySensors {
		...
		telemetry.addData("MotorEncoder", motorRight.getCurrentPosition());
			...
			switch (count) {
				...
				case 2:
					motorLeft.setPower(vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 12000){
						currentPos = motorRight.getCurrentPosition();
				...
				case 3:
					motorLeft.setPower(-vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 2150){//90 degree turn
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;
