...
public class redMountainPark extends PushBotTelemetrySensors {
		...
		telemetry.addData("MotorEncoder", motorRight.getCurrentPosition());
			...
			switch (count) {
				...
				case 0:
					...
					if (motorRight.getCurrentPosition() > currentPos + 1100) {
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;
				...
				case 1:
					motorLeft.setPower(-vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 1300){
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;
				...
				case 2:
					motorLeft.setPower(vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 7000){
						currentPos = motorRight.getCurrentPosition();
						count = 3;//3 is mountain, 5 is floor goal
					}
					break;

				//FIRST SCENARIO turn to orient robot in front of mountain
				case 3:
					motorLeft.setPower(-vl);
					motorRight.setPower(vr);
					if (motorRight.getCurrentPosition() > currentPos + 2000){//90 degree turn
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;

				//FIRST SCENARIO drive onto mountain
				case 4:
					motorLeft.setPower(vl);
					motorRight.setPower(vr);
				...
				//SECOND SCENARIO turn to floor goal
				case 5:
					motorLeft.setPower(vl);
					motorRight.setPower(-vr);
					if (motorRight.getCurrentPosition() < currentPos - 1000) {
						currentPos = motorRight.getCurrentPosition();
						count++;
					}
					break;

				//SECOND SENARIO drive into floor goal
				case 6:
					motorLeft.setPower(vl);
					motorRight.setPower(-vr);
					if (motorRight.getCurrentPosition() < currentPos - 1000) {
